...
(define (test-expression exp)
  ...
  (define stage1 (simplify-ir (desugar stage0)))
  (define stage2 (cps-convert (anf-convert (alphatize (assignment-convert stage1)))))
  (define stage3 (proc->llvm (closure-convert stage2)))

  (define llvm-result (eval-llvm stage3))
  ...
  (if (or
      ...
      #t
      (if (equal? (eval-top-level stage0) (eval-llvm stage3))
          #t
          #f))
  )

(define (test-top-level exp)
  (define stage0 (top-level exp))
  (eval-top-level stage0)
  )

(define (test-llvm exp)
  (define stage0 (top-level exp))
  (define stage1 (simplify-ir (desugar stage0)))
  (define stage2 (cps-convert (anf-convert (alphatize (assignment-convert stage1)))))
  (define stage3 (proc->llvm (closure-convert stage2)))
