...
public:
    ...
    {
	if (*(this->k) == *k)
	    return new ((LLtype*)malloc(sizeof(LLtype))) LLtype(this->k, v, next);
	else if (next)
	    return new ((LLtype*)malloc(sizeof(LLtype))) LLtype(this->k, this->v, next->insert(k, v, cptr));
	else
	{
	    (*cptr)++;
	    const LLtype* const link1 = new ((LLtype*)malloc(sizeof(LLtype))) LLtype(this->k, this->v, 0);
	    const LLtype* const link0 = new ((LLtype*)malloc(sizeof(LLtype))) LLtype(k, v, link1);
	    return link0;                
	}
    }
	...
	else if (this->next)
	{
		...
		return new ((LLtype*)malloc(sizeof(LLtype))) LLtype(this->k, this->v, next);
	}
	else
	    return this;
    }
...
public:	
    ...
    {
	KVtype* copy = (KVtype*)malloc(count*sizeof(KVtype));
	...
	if (h0piece == h1piece)
	{
	    ...
	    KVnext* const node = (KVnext*)malloc(sizeof(KVnext));
	...
	else
	{
	    ...
	    KVnext* const node = (KVnext*)malloc(2*sizeof(KVnext));
	    if (h1piece < h0piece)
	    {
	...
	else
	{
	    ...
	    (*cptr)++;
	    KVnext* const node = (KVnext*)malloc((count+1)*sizeof(KVnext));
...
public:	
    ...
    {
	KVbottom* copy = (KVbottom*)malloc(count*sizeof(KVbottom));
	std::memcpy(copy, old, count*sizeof(KV));
    ...
    static const KVbottom new_inner_node(const u64 h0, const K* const k0, const V* const v0,
					   const u64 h1, const K* const k1, const V* const v1)
    {
	const LLtype* const ll1 = new ((LLtype*)malloc(sizeof(LLtype))) LLtype(k0, v0, 0);
	const LLtype* const ll0 = new ((LLtype*)malloc(sizeof(LLtype))) LLtype(k1, v1, ll1);
	...
	if ((kv.k.bm & 1) == 0)
	{
		...
		(*cptr)++;
		const LLtype* const ll1 = new ((LLtype*)malloc(sizeof(LLtype))) LLtype(key, val, 0);
		const LLtype* const ll0 = new ((LLtype*)malloc(sizeof(LLtype))) LLtype(kv.k.key, kv.v.val, ll1);
		return KVbottom(1, ll0);
	    }
	}
	else //if ((kv.k.bm & 1) == 1)
	{
	    ...
	    {
		(*cptr)++;
	        const LLtype* const ll0 = new ((LLtype*)malloc(sizeof(LLtype))) LLtype(key, val, 0);
		return KVbottom(1, ll0);
	    }
	}
    }
...
public:
	...
	// Make a copy to return; insert at bucket hpiece 
        hamt<K,V>* new_root = (hamt<K,V>*)malloc(sizeof(hamt<K,V>));
